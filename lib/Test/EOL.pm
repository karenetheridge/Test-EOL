...
sub _all_files {
    ...
    my $want_sub = sub {
        ...
        $File::Find::prune = 1, return if $chunks[-1] eq 'CVS' or $chunks[-1] eq '.svn';          # cvs, subversion
        $File::Find::prune = 1, return if $chunks[-2] eq 'blib' and $chunks[-1] eq 'libdoc';      # pod doc
        $File::Find::prune = 1, return if $chunks[-2] eq 'blib' and $chunks[-1] =~ /^man[0-9]$/;  # pod doc
        $File::Find::prune = 1, return if $chunks[-1] eq 'inc';                                   # Module::Install

        return if (File::Spec->splitdir($File::Find::name))[-1] eq 'Build';   # autogenerated Build script
